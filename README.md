...
![Version](https://img.shields.io/badge/version-2.0.1-10A37F)
...
`build.ps1` 会自动从 `wintuner.__version__` 读取版本号，避免源码版本和 EXE 文件版本不一致。当前版本 `2.0.1` 会写入 Windows 文件版本 `2.0.1.0`。
...
`2.0.1` 对页面切换和异常处理做了稳定性修正：

- 修复 `_make_toggle()` 漏导入 `_gl()` / `_sep()`，导致“系统设置”和“高级设置”首次创建时抛出 `NameError` 的问题；
- 页面仍按需创建，但 `QStackedWidget` 的页面槽位保持固定，不再在导航点击时动态删除/重插控件；
- 页面创建、自动状态检测和后台任务回调都增加了异常边界，单个功能异常不会再直接带崩整个 GUI；
- 移除了仅用于装饰的 OpenGL Shader 动态背景，改用 Qt 软件绘制渐变，减少显卡驱动 / OpenGL 合成差异带来的不确定性；
- 状态检测线程并发数从 6 调整为 4，降低页面首次检测时同时启动大量系统查询的瞬时压力。

如果某个页面仍出现异常，WinTuner 会尽量保留主窗口并将详细 traceback 写入：

```text
%TEMP%\WinTuner_error.log
```

排查问题时建议先从 PowerShell 运行源码版：

```powershell
python main.py
```

确认页面切换和功能检测正常后，再执行 `./build.ps1` 构建 Nuitka 发布版。
