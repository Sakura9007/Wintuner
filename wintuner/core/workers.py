...
class MixIn:
    ...
    def _report_callback_exception(self, context):
        ...
        if callable(logger):
            try:
                logger(f'✗ {context}，错误已写入 WinTuner_error.log')
            except Exception:
                pass

    def _safe_callback(self, cb, context):
        def _wrapped(*args):
            try:
                cb(*args)
            except Exception:
                # 绝不能让 Python 异常从 Qt signal/slot 边界逃回 C++。
                self._report_callback_exception(context)
        return _wrapped

    def _run_async(self, func, cb, *args, pool=None):
        sig = _Sig()
        sig.done.connect(self._safe_callback(cb, '后台任务回调异常'))
        self._track_sig(sig)
        (pool or _DETECT_POOL).start(_Worker(func, args, sig))

    def _run_object_async(self, func, cb, *args, pool=None):
        sig = _ObjSig()
        sig.done.connect(self._safe_callback(cb, '后台对象任务回调异常'))
        self._track_sig(sig)
    ...
    def _end_detect_batch(self, rerun):
        ...
        if self._detect_batch_pending:
            self._detect_batch_pending = False
            QTimer.singleShot(0, self._safe_callback(rerun, '状态检测重试异常'))
